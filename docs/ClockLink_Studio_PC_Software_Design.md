...
GUI 第一版已使用 Tkinter 实现，避免增加大型依赖。

当前核心区域：

1. `Connect` 页：HELLO、PING、STATUS、同步时间、读取时间、发送消息、读取消息槽。
2. `Control` 页：闹钟槽读写、日程槽读写、倒计时设置/启动/停止/查询。
3. 全局底部 `通信日志`：固定显示在窗口下半部分，显示每次请求的回复帧、消息正文解码结果和错误信息，并提供缩小、默认、放大三档高度控制。
4. `Connect` 页中部聊天记录：按聊天软件习惯显示 PC 发送、FPGA/mock 回复和系统提示，便于演示消息交互历史。
