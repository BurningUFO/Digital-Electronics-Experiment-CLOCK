...
GUI 第一版已使用 Tkinter 实现，避免增加大型依赖。当前视觉层采用浅色桌面应用风格：顶部品牌栏、白色功能卡片、现代化标签页、带头像和阴影层次的聊天气泡、独立输入栏，以及深色底部日志控制台。
...
4. `Connect` 页聊天记录：PC 发送显示为右侧蓝色气泡，FPGA/mock 回复显示为左侧白色气泡，系统提示居中显示；聊天标题区带 USB-UART 状态标签，消息气泡带头像、时间戳和轻量阴影，便于演示消息交互历史。
...
3. 真实串口 transport 已实现，并带后台读取队列；GUI 可显示 FPGA 主动 `REPLY` 帧。尚未完成 Nexys A7 板级 USB-UART 实测。
