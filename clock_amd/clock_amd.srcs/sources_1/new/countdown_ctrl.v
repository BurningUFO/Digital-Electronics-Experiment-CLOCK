module countdown_ctrl(
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            countdown_run <= 1'b0;
        end else if (tick_1k && countdown_run) begin
            if (qd_pulse && ctrl_sel == 3'b011) begin
                countdown_run <= 1'b0;
            end else if (tick_1h && countdown_one) begin
                countdown_run <= 1'b0;
            end
        end else if (tick_1k && qd_pulse && ctrl_sel == 3'b011 && countdown_nonzero) begin
            countdown_run <= 1'b1;
        end
    end
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            sec_unit_bcd <= 4'd0;
        end else if (tick_1k && edit_sec_unit) begin
            if (sec_unit_bcd == 4'd9) begin
                sec_unit_bcd <= 4'd0;
            end else begin
                sec_unit_bcd <= sec_unit_bcd + 1'b1;
            end
        end else if (tick_1k && run_tick_nonzero) begin
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            sec_ten_bcd <= 4'd0;
        end else if (tick_1k && edit_sec_ten) begin
            if (sec_ten_bcd == 4'd5) begin
                sec_ten_bcd <= 4'd0;
            end else begin
                sec_ten_bcd <= sec_ten_bcd + 1'b1;
            end
        end else if (tick_1k && borrow_sec_ten) begin
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            min_unit_bcd <= 4'd0;
        end else if (tick_1k && edit_min_unit) begin
            if (min_unit_bcd == 4'd9) begin
                min_unit_bcd <= 4'd0;
            end else begin
                min_unit_bcd <= min_unit_bcd + 1'b1;
            end
        end else if (tick_1k && borrow_min_unit) begin
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            min_ten_bcd <= 4'd0;
        end else if (tick_1k && edit_min_ten) begin
            if (min_ten_bcd == 4'd9) begin
                min_ten_bcd <= 4'd0;
            end else begin
                min_ten_bcd <= min_ten_bcd + 1'b1;
            end
        end else if (tick_1k && borrow_min_ten && min_ten_bcd > 4'd0) begin
            min_ten_bcd <= min_ten_bcd - 1'b1;
        end
    end
endmodule
