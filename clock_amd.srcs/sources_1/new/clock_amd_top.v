...
// Nexys A7 板级顶层。
//
// 本模块只负责把开发板物理接口接入工程主线：
// 1. 由 100MHz 板载时钟生成全系统使用的 tick_1k 使能脉冲。
// 2. 实例化 clock 主系统，承接按键、开关、USB-UART、蜂鸣器和显示状态。
// 3. 实例化 ADT7420 温度读取、八位数码管扫描和 SSD1306 OLED 状态副屏。
// 4. 保持顶层端口与 clock_amd.xdc 一一对应；新增/删除端口时必须同步检查约束。
//
// 复位约定：
// - 传给大多数主线 HDL 的 rst 为 CPU_RESETN，低有效。
// - 传给 OLED/ADT7420 子模块的 rst 为 ~CPU_RESETN，高有效。
// -----------------------------------------------------------------------------
