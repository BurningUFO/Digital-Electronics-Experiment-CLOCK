module alarm_ctrl(
    ...
    reg  [2:0] alarm_sec_ten_reg;
    reg  [3:0] alarm_sec_unit_reg;
    reg  alarm_match_d;
    reg  alarm_beep_reg;
    reg  [1:0] beep_state;
    reg  [1:0] beep_index;
    ...
    wire [19:0] cur_alarm_bus;
    wire [19:0] set_alarm_bus;
    wire alarm_match_raw;
    wire alarm_trigger_pulse;
    wire beep_trigger_pulse;

    assign alarm_sec_ten_bcd   = {1'b0, alarm_sec_ten_reg};
    assign alarm_sec_unit_bcd  = alarm_sec_unit_reg;
    ...
    assign set_alarm_bus       = {alarm_hour_ten_reg, alarm_hour_unit_reg,
                                  ...
                                  alarm_sec_ten_reg, alarm_sec_unit_reg};
    assign alarm_match_raw     = alarm_enable_reg & ~|(cur_alarm_bus ^ set_alarm_bus);
    assign alarm_trigger_pulse = alarm_match_raw & ~alarm_match_d;
    assign beep_trigger_pulse  = alarm_trigger_pulse | countdown_done_pulse;
    ...
    always @(posedge clk or negedge rst) begin
        if (!rst) begin
            ...
            alarm_sec_ten_reg   <= 3'd0;
            alarm_sec_unit_reg  <= 4'd0;
            ...
            if (alarm_enable_inc_pulse) begin
                alarm_enable_reg <= 1'b1;
            ...
            if (alarm_hour_inc_pulse) begin
                ...
                if (alarm_hour_ten_reg == 2'd0 &&
                    alarm_hour_unit_reg == 4'd0) begin
                    alarm_hour_ten_reg  <= 2'd2;
                    alarm_hour_unit_reg <= 4'd3;
                end else if (alarm_hour_unit_reg == 4'd0) begin
                    alarm_hour_ten_reg  <= alarm_hour_ten_reg - 1'b1;
                    alarm_hour_unit_reg <= 4'd9;
                end else begin
                    alarm_hour_unit_reg <= alarm_hour_unit_reg - 1'b1;
                end
            end
            ...
            if (alarm_min_inc_pulse) begin
                ...
                if (alarm_min_ten_reg == 3'd0 &&
                    alarm_min_unit_reg == 4'd0) begin
                    alarm_min_ten_reg  <= 3'd5;
                    alarm_min_unit_reg <= 4'd9;
                end else if (alarm_min_unit_reg == 4'd0) begin
                    alarm_min_ten_reg  <= alarm_min_ten_reg - 1'b1;
                    alarm_min_unit_reg <= 4'd9;
                end else begin
                    alarm_min_unit_reg <= alarm_min_unit_reg - 1'b1;
            ...
            if (alarm_sec_inc_pulse) begin
                if (alarm_sec_ten_reg == 3'd5 &&
                    alarm_sec_unit_reg == 4'd9) begin
                    alarm_sec_ten_reg  <= 3'd0;
                    alarm_sec_unit_reg <= 4'd0;
                end else if (alarm_sec_unit_reg == 4'd9) begin
                    alarm_sec_ten_reg  <= alarm_sec_ten_reg + 1'b1;
                    alarm_sec_unit_reg <= 4'd0;
                end else begin
                    alarm_sec_unit_reg <= alarm_sec_unit_reg + 1'b1;
                end
            end else if (alarm_sec_dec_pulse) begin
                if (alarm_sec_ten_reg == 3'd0 &&
                    alarm_sec_unit_reg == 4'd0) begin
                    alarm_sec_ten_reg  <= 3'd5;
                    alarm_sec_unit_reg <= 4'd9;
                end else if (alarm_sec_unit_reg == 4'd0) begin
                    alarm_sec_ten_reg  <= alarm_sec_ten_reg - 1'b1;
                    alarm_sec_unit_reg <= 4'd9;
                end else begin
                    alarm_sec_unit_reg <= alarm_sec_unit_reg - 1'b1;
