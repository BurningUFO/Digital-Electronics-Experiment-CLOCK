...
打包产物 `dist/ClockLinkStudio.exe` 可以直接拷到 Windows 电脑运行。`build/`、`dist/` 和 `artifacts/releases/` 都是生成产物，不提交到 Git；正式分发时把 ZIP 上传到 GitHub Release。

完整发行流程见 `docs/ClockLink_Studio_Release_Guide.md`。
